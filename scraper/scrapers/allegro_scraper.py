"""Allegro.pl scraper implementation."""
...
from scraper.config import ALLEGRO_CONFIG, REQUEST_TIMEOUT, MAX_RETRIES, RETRY_DELAY


class AllegroScraper(BaseScraper):
    """Scraper for Allegro.pl e-commerce website."""
    
    def __init__(self):
        super().__init__()
        self.config = ALLEGRO_CONFIG
    ...
    def search_products(self, query: str, max_results: int = 10) -> List[Product]:
        """Search for products on Allegro.pl"""
        products = []
        
        try:
            # Construct search URL
            search_url = f"{self.config['search_url']}?string={quote(query)}"
            ...
            print(f"Error searching Allegro: {e}")
        
        return products
    
    def get_product_details(self, url: str) -> Optional[Product]:
        """Get detailed product information from Allegro.pl"""
        try:
            ...
            price_elem = soup.select_one('div[class*="price"] span')
            image_elem = soup.select_one('img[data-role="product-image"]')
            ...
            print(f"Error getting product details from Allegro: {e}")
            return None
